...
fn release_workflow_catalog_publication_is_last_and_matrix_gated() -> anyhow::Result<()> {
    ...
    for required in [
        ...
        r#"if [ "$previous" = "0.1.14" ] || [ "$previous" = "0.1.15" ] || [ "$previous" = "0.1.16" ] || [ "$previous" = "0.1.17" ]; then"#,
        r#"if ($previous -eq "0.1.14" -or $previous -eq "0.1.15" -or $previous -eq "0.1.16" -or $previous -eq "0.1.17")"#,
        "Core 0.1.18 repairs authenticated catalog binding in the updater.",
