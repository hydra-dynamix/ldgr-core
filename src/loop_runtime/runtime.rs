...
fn run_loop_after_start(
    ...
    record_run_phase(
        ...
        run_id,
        "running_agent",
        &format!("Running loop agent command for {work_slug}."),
    )?;
    live_progress_phase(
        options,
        run_id,
    ...
    let mut agent = run_loop_agent(artifact_root, options, run_id, work_slug, &rendered_prompt)?;
    if matches!(options.agent, LoopAgent::Agentctl) {
        agent = enrich_agentctl_failure_output(agent);
    }
    let agent_exit_code = agent.exit_code;
    if get_run(connection, run_id)?.status == RunStatus::Running {
        record_run_phase(
            connection,
            run_id,
            "capturing_agent_output",
            &format!("Capturing loop agent output for {work_slug}."),
        )?;
    }
    let agent_output_path = artifact_root.join(format!("loop-run-{run_id}-agent-output.md"));
    let output_argv = agent_output_argv(&options.agent);
    fs::write(
        &agent_output_path,
        agent.to_markdown("Agent output", &output_argv),
    )
    .with_context(|| {
        format!(
    ...
    if let Some(stdout_path) = &agent.stdout_artifact_path {
        let compat_stdout = artifact_root.join(format!("loop-run-{run_id}-agent-stdout.txt"));
        if stdout_path != &compat_stdout {
            fs::copy(stdout_path, compat_stdout)?;
        }
    }
    if let Some(stderr_path) = &agent.stderr_artifact_path {
        let compat_stderr = artifact_root.join(format!("loop-run-{run_id}-agent-stderr.txt"));
        if stderr_path != &compat_stderr {
            fs::copy(stderr_path, compat_stderr)?;
        }
    }
    ...
    let final_status = if options.dry_run {
        RunStatus::Partial
    } else if agent_exit_code == Some(0)
...
fn run_loop_agent(
    ...
    let loop_env = [
        ...
        ("LDGR_LOOP_MAY_CLOSE_RUN", "1"),
    ];
    match &options.agent {
        LoopAgent::DryRun => Ok(ProcessCapture::from_memory(
            None,
            0,
            "dry-run: agent would run here\n".to_owned(),
            String::new(),
        )),
        LoopAgent::Argv(argv) => run_process_with_stdin_env(
            ...
            Some(format!("run={run_id} work={work_slug}")),
        ),
        LoopAgent::Agentctl => {
