...
const RUN_HELP: &str = "Examples:\n  ldgr run start fix-login --command \"cargo test login\"\n  ldgr run finish 7 --status success --notes \"Tests pass.\"\n  ldgr run close 7 --status success --outcome continue --rationale \"Login fixed; add docs.\" --next-slug docs --next-title \"Update docs\" --next-description \"Document token refresh.\"\n  ldgr run close 7 --status success --outcome continue --rationale \"Login fixed; docs already queued.\" --next-slug queued-docs\n\nRuns capture bounded execution. Decisions close the work narrative. --next-slug links an existing active work item, or creates one when --next-title and --next-description are supplied. Stop means the whole project is complete.";
